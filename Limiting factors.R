...
xtitle <- c("","","","","","","","","Arbitrary axis [0-1]","Arbitrary axis [0-1]","Arbitrary axis [0-1]","Arbitrary axis [0-1]")
...
for (i in 1:12){  
  ...
  index <- plot_nr[i]
  Ii <- CalLight(mid_of_month[index],P[mid_of_month[index],],
                  D[mid_of_month[index],],param)
  
  g <- param$gmax * pmin(param$alpha*Ii / sqrt((param$alpha*Ii)**2 + param$gmax**2),N[mid_of_month[index],]/(N[mid_of_month[index],]+param$HN)) 
  
  plot(param$alpha*Ii / sqrt((param$alpha*Ii)**2 + param$gmax**2), param$z, ylim = rev(range(param$z)),xlim = 0:1, type = "l",pch = 19, col = "orange", 
  ...
  lines(N[mid_of_month[index],]/(N[mid_of_month[index],]+param$HN), param$z, ylim = rev(range(param$z)), pch = 19, col = "blue3",lwd = 3,type = "l")
  lines(g/param$gmax, param$z, ylim = rev(range(param$z)), pch = 19, col = "darkseagreen",lwd = 3,type = "l", lty = 3)
  text(0.5,5,month[index])
  
}

legend(0.52,41,legend=c("Relative Light", "Relative Nutrients", "Relative growth"),col=c("orange", "blue3", "darkseagreen"),pch = 19,cex = 0.7)
